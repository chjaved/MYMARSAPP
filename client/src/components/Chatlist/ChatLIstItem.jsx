...
export default function ChatLIstItem({ data, isContactPage = false }) {
  ...
  return (
    ...
    className={`flex cursor-pointer items-center ${
      currentChatUser?.id === data.id && !isContactPage
        ? "glass-effect" // Apply glass effect when the condition is met
        : "hover:glass-effect" // Apply glass effect on hover
    }`}
  ...
  >
      ...
      <div className="min-h-full flex flex-col justify-center mt-3 pr-2 w-full">
        ...
        <div className="flex border-b border-x-blue-400 pb-2 pt-1 pr-2">
          <div className="flex justify-between w-full">
            <span className="text-black line-clamp-1 text-sm ">
              {isContactPage ? (
                data?.about || "\u00A0"
              ) : (
